...
| Rich messages that keep their premium emoji | `read_rich_message` renders `richTextCustomEmoji` (it used to drop every one, which read as "rich messages cannot carry premium emoji" - they can, as `![glyph](tg://emoji?id=N)`), takes a username or `me` like every neighbouring tool, `send_message` takes `rich_rtl` because Telegram does not infer direction, and `rich_files` fills the `files` list on `InputRichMessageHTML`/`Markdown` so `<img>`, `<video>`, `<audio>` and a `tg://document` link resolve - without it the tag is dropped and the message arrives as text. `copy_message` takes `repeat` and `topic_id`, which is the only way to put a RICH message on a recurring schedule - `schedule_message` composes from text and entities and cannot express a table or a photo block. `download_rich_media` fetches the photo, clip or file a block names: the block carries a pointer and the file itself travels beside it in the message's own `photos`/`documents`, so the two are matched; the message is empty, so `download_media` has nothing to work with. Both rich tools left TDLib on 2026-09-21 - `messages.getRichMessage` answers with a MESSAGES container whose `Message.rich_message` holds the body (`docs/adr/0005`), and the rendering moved to `rich_blocks.py`. Body text is sanitised as body text - it went through the display-name sanitiser, which caps at 256 characters and flattens newlines, so a long table cell came back truncated | `tools/rich_messages.py`, `rich_blocks.py`, `tools/messages_relay.py`, `runtime.py` |
...
| Copying a message whole | `copy_message` | `tools/messages_relay.py` |
...
Ten surfaces where the server reported a fact and the tool that should act on
...
| Telegram's own picker offers an account several posting identities | nothing here could name them or use one, so every message went out as the account itself even from a channel that had a linked identity | `list_send_as` reports them with titles and premium gates, `send_as` chooses one per message, `set_default_send_as` makes one stick |
...
4. ~~Split `tools/messages.py` and `runtime.py`~~ **done**, along with `tools/groups.py`,
   ...
   **Two more on 2026-09-12**, both by moving whole tools rather than fragments:
   `tools/messages.py` 1010 -> 705 (`messages_relay.py` - putting a message that
   already exists into another chat: `forward_message`, `forward_messages`,
   `copy_message` and the `_album_batch` both widen an id through) and
   `tools/profile.py` 825 -> 519 (`profile_privacy.py` - the four `_PRIVACY_*`
   translation tables and the two tools that read them).

   The schedule vocabulary `copy_message` shares with `schedule_message`
   (`_as_utc`, `_repeat_seconds`, `REPEAT_PERIODS`) STAYED in `messages.py`,
   because that module is the documented base of the family: siblings import from
   it and it imports nothing back, and the re-export at its foot is a
   compatibility shim rather than a dependency.

   The trap this time was the one `tests/conftest.py` already names - patch the
   module that OWNS each name. `wire_client` patches `get_client` on the module
   object, so a tool that moves keeps talking to the real client while its test
   still passes. Five test files were re-pointed, and re-pointing the fixture was
   not enough: `test_round_trips.py` also reached `_PRIVACY_INPUT_RULES` directly
   and went red until those three references moved too.
